...
public:
    AlgorithmThreatVisualizer() : Node("threat_visualizer")
    {
        ...
        this->declare_parameter("_threat_resolution", 11);
        ...
        image = mess2_plugins::get_threat_field_image(image_threat, colormap);
        ...
        RCLCPP_INFO(this->get_logger(), "generating threat vertices");

        arma::vec x_scaled = arma::linspace(x_scaled_min, x_scaled_max, _image_resolution);
        arma::vec y_scaled = arma::linspace(y_scaled_min, y_scaled_max, _image_resolution);
        auto [x_mesh_scaled, y_mesh_scaled] = mess2_plugins::get_meshgrid(x_scaled, y_scaled);
        scaled_threat = mess2_plugins::generate_threat(x_mesh_scaled, y_mesh_scaled);

        VertexArray vertices = mess2_plugins::get_vertices(scaled_threat, x_mesh_scaled, y_mesh_scaled);

        RCLCPP_INFO(this->get_logger(), "generated threat vertices");

        _vertices_publisher = this->create_publisher<VertexArray>(
            "visualizer/threat/vertices",
            10
        );
        _vertices_publisher->publish(vertices);
    }
private:
