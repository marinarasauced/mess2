...
#include "mess2_msgs/action/ugv_calibrate.hpp"
#include "mess2_plugins/calibration.hpp"
#include "mess2_plugins/utils.hpp"

using TransformStamped = geometry_msgs::msg::TransformStamped;
using Twist = geometry_msgs::msg::Twist;
using Action = mess2_msgs::action::UGVCalibrate;
...
{
public:
    explicit UGVCalibrationServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
    : Node("ugv_calibration_server", options)
    {
        ...
        _vicon_subscription = this->create_subscription<TransformStamped>(
            _vicon_topic,
            10,
            std::bind(&UGVCalibrationServer::_vicon_callback, this, _1)
        );
        ...
        this->_calibration_server = rclcpp_action::create_server<Action>(
            this,
            "calibrate_ugv",
            std::bind(&UGVCalibrationServer::_handle_goal, this, _1, _2),
            std::bind(&UGVCalibrationServer::_handle_cancel, this, _1),
            std::bind(&UGVCalibrationServer::_handle_accepted, this, _1)
        );
    }
...
private:
    ...
    {
        std::thread{std::bind(&UGVCalibrationServer::_calibration_execute, this, std::placeholders::_1), goal_handle}.detach();
    }
    ...
    void _calibration_execute(const std::shared_ptr<GoalHandle> goal_handle)
    {
        ...
        RCLCPP_INFO(this->get_logger(), "transitioning turtlebot3 forward for one second");
        rclcpp::Time tic_ = this->get_clock()->now();
        rclcpp::Time toc_ = tic_ + rclcpp::Duration::from_seconds(1.0);
        Twist cmd_vel_msg;
        cmd_vel_msg.linear.x = 0.5;
        while (tic_ < toc_)
        {
            rclcpp::Time tic_ = this->get_clock()->now();
            _cmd_vel_publisher->publish(cmd_vel_msg);
        }
